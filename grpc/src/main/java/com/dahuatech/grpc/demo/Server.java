...
public class Server extends InterfaceServiceGrpc.InterfaceServiceImplBase {
    ...
    public void get(Request request, StreamObserver<Response> responseObserver) {
        ...
        try {
            response = Response.newBuilder().addStudent(Student.newBuilder().setName(request.getName()).setAge(request.getAge()).setAddress("ahut").build())
                    .addStudent(Student.newBuilder().setName(request.getName()).setAge(request.getAge()).setAddress("ahut").build())
                    .addStudent(Student.newBuilder().setName(request.getName()).setAge(request.getAge()).setAddress("ahut").build())
                    .build();
        } catch (Exception e) {
            response = Response.newBuilder().addStudent(Student.newBuilder().setName(request.getName()).setAge(request.getAge()).setAddress("ahut").build())
